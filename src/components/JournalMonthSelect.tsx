...
type JournalMode = "all" | "today" | "week" | "month";
...
function firstTradingWeekStart(year: number, month: number): string {
  const firstOfMonth = isoDate(year, month, 1);
  const weekday = isoWeekday(firstOfMonth);
  const daysUntilMonday = weekday === 0 ? 1 : weekday === 1 ? 0 : 8 - weekday;
  return isoAddDays(firstOfMonth, daysUntilMonday);
}

export default function JournalMonthSelect({
  mode,
  selectedDate,
}: {
  mode: JournalMode;
  ...
  function applyMonth() {
    ...
    const targetMode = mode === "week" ? "week" : "month";
    params.set("preset", targetMode);

    if (targetMode === "week") {
      const from = firstTradingWeekStart(year, month);
      params.set("from", from);
      params.set("to", isoAddDays(from, 4));
    } else {
