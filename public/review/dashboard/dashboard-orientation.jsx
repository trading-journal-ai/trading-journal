...
function TopNav({ active = 'Dashboard' }) {
  const items = ['Dashboard', 'Journal', 'Calendar', 'Trades', 'Analytics'];
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 30, padding: '18px 34px', borderBottom: '1px solid var(--border)' }}>
      <span style={{ fontSize: 16, fontWeight: 700, color: 'var(--ink)', letterSpacing: '-0.01em', whiteSpace: 'nowrap' }}>Trading Journal AI</span>
      ...
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 7, height: 34, padding: '0 13px', borderRadius: 7, border: '1px solid var(--border)', fontSize: 13, fontWeight: 600, color: 'var(--body)' }}>
        <span style={{ whiteSpace: 'nowrap' }}>Paper Account</span>
...
function CmdCell({ label, value, sub, valueColor, accent, first, onClick, open }) {
  return (
    <div onClick={onClick} style={{ padding: '16px 20px', borderLeft: first ? 'none' : '1px solid var(--hair)', position: 'relative', cursor: onClick ? 'pointer' : 'default', background: open ? 'var(--surface-2)' : 'transparent', transition: 'background .15s' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 9 }}>
        ...
        {onClick && (
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontFamily: 'var(--mono)', fontSize: 9.5, letterSpacing: '.04em', color: 'var(--accent)', fontWeight: 600 }}>
            {open ? 'CLOSE' : 'CHECK IN'}
            <svg width="9" height="9" viewBox="0 0 11 11" fill="none" stroke="var(--accent)" strokeWidth="1.7" strokeLinecap="round" style={{ transform: open ? 'rotate(180deg)' : 'none', transition: 'transform .15s' }}><path d="M2 4l3.5 3.5L9 4" /></svg>
          </span>
        )}
      </div>
...
function MorningRecap({ showMarket, gap }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap }}>
      <div>
        <CommandStrip cells={[
          { label: 'Phase', value: phaseMeta.label, sub: phase === 'pre' ? 'Opens in 1h 18m' : 'Live', accent: true, onClick: () => setCheckIn((v) => !v), open: checkIn },
        ...
        {checkIn && <PhaseCheckIn phase={phase} onPhase={setPhase} />}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: showMarket ? '1fr 312px' : '1fr 312px', gap: 44, alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap }}>
          ...
          {/* Opportunity set — market context + top-gainers review (the loop) */}
          <OpportunitySet market={market} onMarket={setMarket} />
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap }}>
          ...
          <OpenSection eyebrow="Session log" right={
            <span style={{ fontFamily: 'var(--mono)', fontSize: 10.5, color: 'var(--faint)' }}>Saved · look back anytime</span>
          }>
            <SessionLog />
          </OpenSection>
        </div>
