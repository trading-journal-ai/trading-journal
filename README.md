...
Nearly every line of code in this project was generated by AI agents working
against the docs in this repo. That is not trivia — it is the intended way to
extend the app. The documentation is written to be agent-legible: contracts,
boundaries, and decision records live in `docs/`, so an AI coding agent (Claude
Code, Codex, or similar) can pick up a feature with the same context a
maintainer would have.

If you want a feature, the workflow is: open the repo in your agent, point it
at the relevant doc below, and describe what you want. [`AGENTS.md`](AGENTS.md)
gives agents the working rules (validation policy, design rules, verification
commands). Two extension paths matter most:

### Add A Broker Adapter

This is the most likely reason you are here: your broker's export does not
import yet. The import pipeline was built for exactly this — broker-specific
logic stays in one adapter, and everything downstream (journal, charts,
reports, coach) only ever sees the canonical trade format.

Breadcrumbs, in the order an agent should read them:

1. [`docs/import/IMPORT_ARCHITECTURE.md`](docs/import/IMPORT_ARCHITECTURE.md) —
   the canonical pipeline, adapter responsibilities, diagnostic contract, and
   confidence levels. The "Multi-Broker Expansion" section is the checklist.
2. [`docs/import/TRADE_IMPORT_BEHAVIOR.md`](docs/import/TRADE_IMPORT_BEHAVIOR.md) —
   what an execution and a trade mean here: flat-to-flat grouping, dedupe,
   append-only sync, and the edge-case matrix your adapter must respect.
3. `src/lib/import/` — the working examples: `inspect.ts` (source detection),
   `tos.ts` and `das.ts` (existing adapters), `normalize.ts`, `match.ts`
   (trade reconstruction), `persist.ts`. A new adapter follows the same shape.

The working loop:

```bash
# 1. Put your export under data/evals/ (gitignored; never commit real exports)
# 2. See what the app can already read from it
npm run broker:inspect -- --file path/to/your-export.csv
# 3. Build/iterate on the adapter with your agent, then preview the normalized output
npm run broker:normalize -- --file path/to/your-export.csv
```

Adapter ground rules (from the architecture doc): detect your format
explicitly, parse only your columns, emit warnings instead of hiding
uncertainty, and never invent missing facts — if your export lacks fill-level
data, label the confidence honestly rather than faking precision. A good first
prompt to an agent: *"Read docs/import/IMPORT_ARCHITECTURE.md and
src/lib/import/das.ts, then add an adapter for this CSV shape"* — plus a few
anonymized sample rows.

### Tune The Coach

The coach is deliberately split into parts you can tune independently:

1. [`docs/coach/COACH_ARCHITECTURE.md`](docs/coach/COACH_ARCHITECTURE.md) —
   start here. The key distinction: the **coach constitution** (identity,
   guardrails, methodology — ships with the app) versus **your playbook**
   (setups, rules, vocabulary — your data, editable). Tuning usually means
   editing the second, not the first.
2. The pillar docs are the coach's knowledge:
   [`docs/coach/SETUPS.md`](docs/coach/SETUPS.md) (what you trade — authored in
   *your* vocabulary so the coach speaks it back),
   [`docs/coach/PSYCHOLOGY.md`](docs/coach/PSYCHOLOGY.md) (process patterns),
   [`docs/coach/VOICE.md`](docs/coach/VOICE.md) (how the coach talks:
   plain-language read, exact-number receipt, one next rep).
3. `src/lib/coach/` — the determinism boundary in code: `reviewEngine.ts`
   computes every number; the model only narrates. If you want the coach to
   consider something new, the path is usually "add a deterministic fact to
   the payload," not "ask the model to estimate it."

Before trusting a tuned coach, run it against known cases:

```bash
npm run coach:eval
```

[`docs/coach/PRIVATE_EVALS.md`](docs/coach/PRIVATE_EVALS.md) explains the
local-only eval format — real trading data stays in gitignored `data/evals/`,
and each case states what the coach *should* and *should not* say. Change the
prompt or pillars, re-run the evals, and you have a regression test for
coaching quality instead of a vibe check.

### Ground Rules For Agent Changes

Whatever you build, finish the loop the same way the existing code was built:
...
Use `npm run verify:full` before a PR. Keep real broker data out of commits,
keep broker-specific logic inside adapters, and keep the coach's numbers
deterministic. If your change touches a documented contract, update the doc in
the same PR — the docs being trustworthy is what makes this workflow possible.
